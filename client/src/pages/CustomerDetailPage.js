...
//     try {
//       await axios.delete(`https://customer-management-app-qqqn.onrender.com/api/customers/${id}`);
//       navigate('/customers');
//     } catch (err) {
...
function CustomerDetailPage() {
  ...
  const fetchAddresses = async () => {
    try {
      const response = await axios.get(`https://customer-management-app-qqqn.onrender.com/api/customers/${id}/addresses`);
      setAddresses(response.data.data);
    } catch (err) {
      setError('Failed to fetch addresses');
      console.error('Error fetching addresses:', err);
